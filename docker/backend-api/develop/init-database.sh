...
if [ -d /backup ] && [ "$(ls -A /backup/backup_develop_*.sql 2>/dev/null)" ]; then
    echo "Found SQL files, getting latest backup..."

    # List backup files
    ls -la /backup/backup_develop_*.sql

    # Find the latest backup file
    LATEST_BACKUP=$(ls -t /backup/backup_develop_*.sql 2>/dev/null | head -1)
    ...
    if [ -n "$LATEST_BACKUP" ] && [ -f "$LATEST_BACKUP" ]; then
        ...
        mysql -h db -u develop_user develop_db < "$LATEST_BACKUP"

        if [ $? -eq 0 ]; then
            echo "MySQL restore command succeeded"

            # Check if data was inserted
            USER_COUNT=$(mysql -h db -u develop_user -pdevelop_pass develop_db -e "SELECT COUNT(*) as user_count FROM tbl_users;" 2>/dev/null | tail -1)
